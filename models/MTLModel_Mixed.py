...
from .networks import StereographicTransformer, StereographicLogits, StereographicLayerNorm, StereographicAct, StereographicLinear

__all__ = ['MTLModel_Mixed']


class MTLModel_Mixed(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        self.encoded_nodes = self.encoder(depot_xy.to(self.device), node_xy_demand_tw.to(self.device))
    ...
    def forward(self, state, selected=None):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            attr = torch.cat((state.load[:, :, None], state.current_time[:, :, None], state.length[:, :, None], state.open[:, :, None]), dim=2).to(self.device)
            # shape: (batch, pomo, 4)
            probs = self.decoder(encoded_last_node, attr, ninf_mask=state.ninf_mask.to(self.device))
...
class MTL_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.embedding_depot_mixed = StereographicLinear(self.manifold, embedding_dim, 128, 8)
        self.embedding_node_mixed = StereographicLinear(self.manifold, embedding_dim, 128, 8)
        
        self.layers = nn.ModuleList([EncoderLayer(**model_params) for _ in range(encoder_layer_num)])
        self.mixed_layers = nn.ModuleList([StereographicLinear(self.manifold, embedding_dim, 128, 8) for _ in range(encoder_layer_num)])
    ...
    def forward(self, depot_xy, node_xy_demand_tw):
        ...
        # shape: (batch, 1, embedding)
        embedded_depot_mixed = self.manifold.expmap0(embedded_depot, self.c_list)
        embedded_depot_mixed = self.embedding_depot_mixed(embedded_depot_mixed, self.c_list)

        embedded_node = self.embedding_node(node_xy_demand_tw)
        # shape: (batch, problem, embedding)
        embedded_node_mxied = self.manifold.expmap0(embedded_node, self.c_list)
        embedded_node_mxied = self.embedding_node_mixed(embedded_node_mxied, self.c_list)
        
        embedded_depot = self.manifold.logmap0(embedded_depot_mixed, self.c_list) 
                       
        embedded_node = self.manifold.logmap0(embedded_node_mxied, self.c_list) 
        ...
        for layer_index, layer in enumerate(self.layers):
            mixed_out = self.manifold.expmap0(out, self.c_list_layer[layer_index])
            mixed_out = self.mixed_layers[layer_index](mixed_out, self.c_list_layer[layer_index])
            out_ = self.manifold.logmap0(mixed_out, self.c_list_layer[layer_index])
            out = self.a[layer_index]*out + self.b[layer_index]*out_
            out = layer(out)
