...
class Trainer:
    ...
    def _train_one_batch(self, data, env):
        ...
        while not done:
            ...
            # Store minimal state info (for guidance loss computation)
            if self.model.use_diffusion_guidance:
                # Create visited mask from ninf_mask (already computed)
        ...
        if self.lambda_guidance > 0.0 and self.model.use_diffusion_guidance and len(action_list) > 0:
            try:
                ...
                for step_idx in sampled_indices:
                    # Reconstruct minimal state for guidance
                    visited_mask = visited_mask_list[step_idx]
                    encoded_last = encoded_last_list[step_idx]
                    step_attr = attr_list[step_idx]
                    step_action = action_list[step_idx]
                    
                    # Create minimal state dict
                    minimal_state = {
                        'visited_mask': visited_mask,
                        'encoded_last': encoded_last,
                        'attr': step_attr
                    }
                    
                    # Compute guidance loss
                    step_guidance_loss = self.model.compute_guidance_loss(
                        state=minimal_state,
                        selected_action=step_action,
                        reward=reward,
                        baseline=baseline
                    )
                    guidance_loss_list.append(step_guidance_loss)
                
                # Average across sampled steps
                guidance_loss_mean = sum(guidance_loss_list) / len(guidance_loss_list)
