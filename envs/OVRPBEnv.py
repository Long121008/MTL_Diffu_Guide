...
class OVRPBEnv:
    ...
    def clone(self, deep_clone_history=False):
        ...
        # Bypass __init__ để tránh lãng phí
        cloned = OVRPBEnv.__new__(OVRPBEnv)
        
        # Gán các thuộc tính config (immutable)
        cloned.env_params = self.env_params
        cloned.problem = self.problem
        cloned.backhaul_ratio = self.backhaul_ratio # <-- Thuộc tính riêng của OVRPB
        ...
        cloned.pomo_size = self.pomo_size # <-- Gán pomo_size đã được tính toán
        ...
        # reset_state (dataclass) được coi là immutable sau khi load
        ...
        # 'open' có thể không dùng ở CVRP, nhưng sẽ dùng ở VRPTW
        ...
        # 'open' trong step_state là một tensor (torch.ones)
