...
class OVRPBLTWEnv:
    ...
    def clone(self, deep_clone_history=False):
        ...
        # THUỘC TÍNH MỚI CỦA BINH ĐOÀN NÀY (OVRPBLTW):
        cloned.route_limit = self.route_limit
        ...
        cloned.current_time = self.current_time.clone() if self.current_time is not None else None # <-- Rất quan trọng cho TW
        cloned.current_coord = self.current_coord.clone() if self.current_coord is not None else None
        cloned.length = self.length.clone() if self.length is not None else None # <-- Rất quan trọng cho Length Limit
